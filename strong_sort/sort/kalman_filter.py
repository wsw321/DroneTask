...
class KalmanFilter(object):
    ...
    def initiate(self, measurement):
        ...
        covariance = np.diag(np.square(std))  # 8*8
        return mean, covariance

    def predict(self, mean, covariance, range):
        ...
        # std_pos = [i * np.exp(range) for i in std_pos]
        # std_vel = [i * np.exp(range) for i in std_vel]  # NOTICE:ADD
